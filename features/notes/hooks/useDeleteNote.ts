...
export function useDeleteNote() {
    ...
    return useMutation({
      ...
      onMutate: async (noteId: string) => {
        // Cancel any outgoing refeches so they don't overwrite our optimistic update
        await queryClient.cancelQueries({ queryKey: ["notes"] });

        // Snapshot the previous notes array value before we alter it
      ...
      // Step 3: Always refetch after success or error to stay perfectly synced with server truth
      onSettled: () => {
